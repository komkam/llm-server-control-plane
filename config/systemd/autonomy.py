...
def restart_allowed_service(service: str) -> tuple[bool, str]:
    ...
    detail = result.stdout.strip() or result.stderr.strip() or "restart requested"
    if result.returncode != 0:
        return False, detail

    # A successful systemctl command does not prove that the service recovered.
    for attempt in range(1, POST_RESTART_ATTEMPTS + 1):
        time.sleep(POST_RESTART_DELAY_SECONDS)
        healthy, health_detail = check_service(service)
        if healthy:
            return True, f"{detail}; verified after restart: {health_detail}"
        detail = f"post-restart check {attempt}/{POST_RESTART_ATTEMPTS}: {health_detail}"
    return False, detail
