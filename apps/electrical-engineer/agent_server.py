"""OpenAI-compatible HTTP facade for the proposal-only Electrical Engineering Agent."""
...
from electrical_agent import run_electrical_agent

HOST = "127.0.0.1"
PORT = 5301
PUBLIC_MODEL_NAME = "electrical-engineer"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            return
        try:
            answer = run_electrical_agent(prompt)
        except Exception as exc:
            self.send_json(HTTPStatus.BAD_GATEWAY, {"error": f"electrical agent failed: {exc}"})
            return
